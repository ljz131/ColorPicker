...
class ColorPickerView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : LinearLayout(
        context, attrs, defStyleAttr), ColorObservable {
    ...
    private fun updateObservableOnDuty() {
        ...
        if (observers != null) {
            for (observer in observers!!) {
                ...
                observer!!.onColor(observableOnDuty!!.color, fromUser = false,
    ...
    init {
        ...
        val enableBrightness = typedArray.getBoolean(R.styleable.ColorPickerView_enableBrightness,
                true)
        onlyUpdateOnTouchEventUp = typedArray.getBoolean(
                R.styleable.ColorPickerView_onlyUpdateOnTouchEventUp, false)
        typedArray.recycle()
        colorWheelView = ColorWheelView(context)
        colorWheelView.isFocusable = true
        colorWheelView.setBackgroundResource(R.drawable.selector_red_btn_border_2019ver)
